...
| Indicador por município | `br_inep_avaliacao_alfabetizacao_municipio.csv.gz` | ano + município + rede | `ano`, `id_municipio`, `rede` | INEP / Base dos Dados |
...
> ⚠ **A fonte municipal não traz `sigla_uf`.** Ela é identificada apenas por
> `id_municipio`. A UF é derivada na Silver pelo join com `bronze.municipio`
> (notebook 03, seção 1b). **Nunca leia `sigla_uf` de
> `bronze.avaliacao_alfabetizacao_municipio`**: a ingestão aplica schema explícito
> por posição, então essa coluna carrega o valor de outro campo, ou nulo.
...
| `rede` | int | `0` total, `2` estadual, `3` municipal, `5` privada. | Contrato §4 |
...
| Tabela | Descrição |
|---|---|
| `silver.medicoes_aprovadas` | Medições aprovadas. **Fonte dos marts principais.** |
| `silver.alunos_aprovados` | Alunos aprovados. **Fonte do mart de distribuição.** |
...
> filtro, médias misturam dado real com simulado, o upsert do MongoDB perde
> documentos por colisão de chave e o ML treina duas vezes sobre o mesmo município.
...
| `gold.meta_vs_resultado` | ano + território + rede + fonte | Cobre os dois grãos. Traz `gap_meta`, `atingiu_meta`, `atingiu_meta_brasil`. Fonte das metas do dashboard. |
...
- A fonte do INEP por UF tem **145 registros**, cobrindo **2023 e 2024**, sem
  duplicidade. A fonte municipal tem **23.995 registros**, nos mesmos dois anos.
...
- `taxa_alfabetizacao` chega em 0–100 nas fontes batch e em 0–1 no streaming; a
