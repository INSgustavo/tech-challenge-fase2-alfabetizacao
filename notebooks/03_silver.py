...
# MAGIC
# MAGIC **A fonte municipal é identificada só por `id_municipio`: ela não traz
# MAGIC `sigla_uf`.** A UF é derivada aqui pelo join com a dimensão territorial
...
if existe("bronze", "avaliacao_alfabetizacao_municipio"):
    ...
    # Chave de resolução território → UF. É a fonte de verdade da UF do fato
    # municipal: qualquer sigla_uf que venha da Bronze é descartada, porque a
    # ingestão aplica schema explícito por posição e pode tê-la preenchido com
    # o valor de outra coluna quando o CSV não traz o campo.
    uf_por_municipio = (
    ...
    print(f"✓ dado oficial municipal encontrado: {total_mun:,} registros "
          f"| {total_mun - sem_uf:,} com UF resolvida pela dimensão")
    if sem_uf:
        print(f"⚠ {sem_uf:,} registros sem correspondência em bronze.municipio — "
              "seguem para a Silver sem UF e o Quality Gate (06) os acusa na "
...
def reconciliar_meta(tabela, meta_df, chaves):
    if meta_df is None:
        return
    consumidas = meta_df.join(chaves_fato.select(*chaves).distinct(), chaves, "inner")
    orfas = meta_df.join(chaves_fato.select(*chaves).distinct(), chaves, "left_anti")
    total, n_consumidas, n_orfas = meta_df.count(), consumidas.count(), orfas.count()
    print(f"\nbronze.{tabela}: {total:,} metas | {n_consumidas:,} associadas a um fato "
          f"| {n_orfas:,} órfãs")
    if n_orfas:
...
reconciliar_meta("meta_uf", meta_uf if existe("bronze", "meta_uf") else None, ["ano", "sigla_uf"])
reconciliar_meta("meta_municipio",
                 meta_mun if existe("bronze", "meta_municipio") else None,
                 ["ano", "id_municipio"])

# Contrapartida: fatos que ficaram sem meta. Junto com o bloco acima, fecha a
# conta nos dois sentidos — nenhuma meta se perde sem aviso, nenhum fato fica
# sem meta sem aviso.
...
if n_sem_meta:
    ...
    print(f"\n⚠ {n_sem_meta:,} fatos sem meta associada (anos: {anos_sem_meta}) — "
          "esses registros aparecem como 'Meta indisponível' no dashboard.")
...
# MAGIC
# MAGIC `fonte_dados = 'simulado'`: são microdados simulados, e a marca acompanha o
# MAGIC dado até o painel.
...
if existe("bronze", "alunos"):
    ...
    spark.sql(f"COMMENT ON TABLE {tbl('silver', 'alunos_proficiencia')} IS "
              "'Proficiência por aluno (microdados SIMULADOS), corte 743 aplicado no grão de aluno. "
              "Grão: aluno_id + ano. Fonte da gold.distribuicao_proficiencia.'")
