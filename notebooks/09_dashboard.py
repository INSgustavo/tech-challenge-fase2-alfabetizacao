...
# dimensões e o nome do município já chegam integrados nos marts — o dashboard
# não lê a Bronze para reconstruí-los.
...
# alimenta nenhum indicador educacional. Todo número educacional daqui vem da Gold.
...
# homônimas a um DataFrame que já tem `regiao` deixaria a referência ambígua no
# groupBy seguinte.
...
# Com "Todos" os anos, a média também atravessa os anos do período.
uf_ano = com_regiao(
...
metas_uf_gold = meta_vs_resultado.filter(F.col("grao") == "uf")
...
# resultado na mesma base de comparação.
meta_uf_recorte = (
...
# KPIs municipais. Dois recortes obrigatórios no mart meta_vs_resultado:
...
# o problema é no join da Silver e deve aparecer lá — não ser mascarado aqui.
